...
int main() {
	...
	struct sockaddr_in servaddr, client; //sockaddr_in yapýsý IPv4 tabanlý soket adreslerini temsil eder.Structýn içerisinde IP adresi, port numarasý ve IPv4 adresi yer alýr.
	servaddr.sin_family = AF_INET;
	inet_pton(AF_INET, "10.10.220.195", &(servaddr.sin_addr));
	...
	char buffer[2048];
	int clientMessageLength = sizeof(client);
	int receivedData = recvfrom(myServerSocket, (char*)buffer, sizeof(buffer), 0, (struct sockaddr*)&client, &clientMessageLength); //recvfrom() , alýnan verinin uzunluðunu byte cinsinden döndürür.
	...
	while (true) {
		...
		if (recvfrom(myServerSocket, (char*)buffer, sizeof(buffer), 0, (struct sockaddr*)&client, &clientMessageLength) != -1) {
